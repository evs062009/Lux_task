    ...
    private static int [] getSortedArr(int [] ints1, int [] ints2){
        ...
        for (int resultIndex = 0, ints1Index = 0, ints2Index = 0; resultIndex < result.length; resultIndex++) {
            ...
            if (ints1Index == ints1.length){
                for (int fillIndex = resultIndex; fillIndex < result.length; fillIndex++, ints2Index++) {
                    result[fillIndex] = ints2[ints2Index];
                }
                break;
            }

            //объединить с предыдущим в 1 метод
            if (ints2Index == ints2.length){
                for (int fillIndex = resultIndex; fillIndex < result.length; fillIndex++, ints1Index++) {
                    result[fillIndex] = ints1[ints1Index];
                }
                break;
            }
